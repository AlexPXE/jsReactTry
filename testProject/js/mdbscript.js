...
const personalMovieDB = {
    ...
    writeYourGenres: function(){        
        ...
        for(let ind = 1; ind < 4;){   
            answ = (prompt(`Ваш любимый жанр под номером ${ind}:`, '')).toLowerCase();
                                 
            if(RegExp(/^\p{L}/, 'u').test(answ)){   
                ind++;
                this.genres.push(answ) ;
            } else {
                alert("Все плохо, я не понимаю, что тут написано!");
            }            
        }
        this.genres.forEach((val, ind) => console.log(`Любимый жанр №${ind+1}: ${val};`));
    },
